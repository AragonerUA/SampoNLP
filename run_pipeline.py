...
def print_detailed_score_distribution(scores: Dict[str, float], auto_threshold: float):
    ...
    for lower_bound in sorted_bins:
        ...
        if not gap_found and auto_threshold >= lower_bound and auto_threshold < upper_bound:
            print("-" * 80)
            print(f"--- Auto-threshold ({auto_threshold:.4f}) is within this range ---".center(80))
            print("-" * 80)
            gap_found = True
...
def main():
    ...
    try:
        ...
        try:
            ...
            first_low_score_reason = next(entry[1] for entry in results.discarded if "Low atomicity score" in entry[1])
            ...
            print("No tokens found discarded due to low score. Histogram may be inaccurate.")
