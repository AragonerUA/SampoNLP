...
def _calculate_frequencies(root_candidates: Set[str], corpus_path: str, min_frequency: int) -> Set[str]:
    """
    ...
    valid_roots = {root for root in root_candidates if word_counts.get(root, 0) >= min_frequency}
    
    print(f"{len(valid_roots)} out of {len(root_candidates)} roots passed the statistical check.")
    return valid_roots

def _calculate_productivity(affix_candidates: Set[str], corpus_path: str, min_productivity: int) -> Set[str]:
    """
    ...
    valid_affixes = {
        affix for affix, words in productivity_scores.items() 
        if len(words) >= min_productivity
    }
    
    print(f"{len(valid_affixes)} out of {len(affix_candidates)} affixes passed the statistical check.")
