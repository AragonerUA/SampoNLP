...
class CleaningResults:
    ...
    def load_from_files(cls, output_dir: str):
        ...
        with open(os.path.join(output_dir, 'cleaned_morphemes.txt'), 'r', encoding='utf-8') as f:
            morphemes = [line.strip() for line in f]
        with open(os.path.join(output_dir, 'discarded_log.tsv'), 'r', encoding='utf-8') as f:
            lines = f.readlines()[2:]
            discarded = [tuple(line.strip().split('\t', 1)) for line in lines]
        with open(os.path.join(output_dir, 'final_scores.json'), 'r', encoding='utf-8') as f:
...
class MorphemeCleaner:
    ...
    def __init__(self, language: str = 'uralic', **kwargs):
        ...
        self._one_char_whitelists = {
            'finnish': {'n', 't'}, 'estonian': {'d', 's', 't'},
            'hungarian': {'t', 'k', 'm', 'd', 'i', 'a', 's'},
            'uralic': {'n', 't', 'd', 's', 'k', 'm', 'i', 'a'}
        }
        self.one_char_whitelist = self._one_char_whitelists.get(self.language, self._one_char_whitelists['uralic'])
        ...
        self.allowed_chars_pattern = self._allowed_chars_regex.get(
            self.language,
            self._allowed_chars_regex['uralic']
        )
        ...
        print(f"Using alphabet validation pattern: {self.allowed_chars_pattern.pattern}")

    def _initial_classification_and_scoring(self, raw_morphemes_path: str) -> Tuple[Set[str], Set[str], Dict[str, float], List[Tuple[str, str]]]:
        ...
        for line in tqdm(lines, desc="Step 1.1: Filtering"):
            ...
            elif re.search(r'[][\^$]', token): reason = "Regular expression"
            elif not any(c.isalpha() for c in token): reason = "Punctuation or number"
            elif '.' in token: reason = "URL or file"
            elif token.isnumeric(): reason = "Number"
            elif token.isupper() and len(token) > 1: reason = "Abbreviation"
            elif token[0].isupper(): reason = "Proper noun"
        ...
        print(f"Step 1 complete: {len(final_roots)} roots, {len(final_affixes)} affixes passed filters.")
    ...
    def process(self, raw_morphemes_path: str, output_dir: str) -> CleaningResults:
        root_candidates, affix_candidates, scores, junk = self._initial_classification_and_scoring(raw_morphemes_path)
